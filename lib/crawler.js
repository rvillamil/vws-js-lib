...
exports.crawlVideoPremieres = function (limit, onShowDataCrawled) {
    ...
    return showLinksCrawlerDescargas2020.crawlLinksFrom(constantsDescargas2020.URL_BASE_VIDEOPREMIERES_HD, limit, '.pelilist li a')
        .then(linkChainedToShowDataList => {
            return torrentLinksCrawlerDT.crawlLinksFrom(constantsDontorrent.URL_BASE_BILLBOARDFILMS, limit, 'pelicula')
...
/**
 * Crawl TVShows for all torrent sites
 *
...
exports.crawlTVShows = function (limit, onShowDataCrawled) {
    ...
    return showLinksCrawlerDescargas2020.crawlLinksFrom(constantsDescargas2020.URL_BASE_TVSHOWS_HD, limit, '.pelilist li a')
        .then(linkChainedToShowDataList => {
            return torrentLinksCrawlerDT.crawlLinksFrom(constantsDontorrent.URL_BASE_TVSHOWS_HD, limit, 'serie')
                .then(linkChainedToTorrentLinkList => {
                    // Concatemos los arrays
                    var newFullLinkChainedList = _concatLists(linkChainedToShowDataList, linkChainedToTorrentLinkList, limit)
                    //console.log(`newFullLinkChainedList - ${JSON.stringify(newFullLinkChainedList)}`)
                    return _doCrawlTVShow(newFullLinkChainedList, onShowDataCrawled)
                })
        }).catch(err => {
...
exports.crawlTVShowCollectionsBy = function (limit, showCollectionList) {
    console.log(`crawler->crawlTVShowCollections() from showCollectionList --> '${JSON.stringify(showCollectionList)}'`)
    ...
    var actions = showCollectionList.map(showCollection => {
        ...
        // ---> TODO: Esto de poner una URL asi no mola...creo que debería de venir en los elmentos de la showcollection para ver de donde se carga

        if (showCollection.domain == constantsDontorrent.DOMAIN) {
            return showCollectionCrawlerDT.crawlDataShowCollection(
                showCollection.url, limit)
        }
    ...
    if (linkToShowData.domain == constantsDescargas2020.DOMAIN) {
        ...
        return tvshowCollectionCrawlerDescargas2020.crawlDataShowCollection(linkToShowData.from, 1)
            .then(showCollection => {
                ...
                console.error(`ERROR! - _doCrawlTVShowAndSearchFrom on domain ${constantsDescargas2020.DOMAIN}: ${err}`)
            })
    } if (linkToShowData.domain == constantsDontorrent.DOMAIN) {

        return showCollectionCrawlerDT.crawlDataShowCollection(linkToShowData.from, 1)
            .then(showCollection => {
                const show = showCollection.shows[0]
                //console.log(`_doCrawlTVShowAndSearchFrom - show '${show.title}'. Error code: '${show.error}' `)
                return _doSearchShowInXMDB(show, 'tv')
            }).catch(err => {
                console.error(`ERROR! - _doCrawlTVShowAndSearchFrom on domain ${constantsDontorrent.DOMAIN}: ${err}`)
            })
    }
}
