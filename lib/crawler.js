...
function _doCrawlAndSearchFrom(linkToShowData) {
    return tumejortorrent.crawlDataShow(linkToShowData.current, linkToShowData.from)
        .then(show => {
            return _doSearchInTMDB(show, 'movie');
        })
        .then(showTMDB => {
...
function _doCrawlTVShowAndSearchFrom(linkToShowData) {
    return tumejortorrent.crawlDataShow(linkToShowData.current, linkToShowData.from)
        .then(show => {
            return _doSearchInTMDB(show, 'tv');
        })
        .then(showTMDB => {
            return _doSearchInOMDB(showTMDB);
        })
...
function _doSearchInTMDB(show, kind) {
    var theYear = show.year
    if (kind == 'tv') {
        theYear = null
    } else {}
    return tmdb.searchShow(show.title, theYear, kind)
        .then(tmdbShow => {
