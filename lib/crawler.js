...
exports.crawlBillboardFilms = function (limit, onShowFoundEvent) {
    return tumejortorrent.crawlURLsWithBillboardFilms(limit)
        .then(urlList => {
            ...
            console.error(`ERROR! - crawlURLsWithBillboardFilms: '${err}'`)
        });
}
...
exports.crawlVideoPremieres = function (limit, onShowFoundEvent) {
    return tumejortorrent.crawlURLsWithVideoPremieres(limit)
        .then(urlList => {
            ...
            console.error(`ERROR! - crawlURLsWithVideoPremieres: '${err}'`)
        });
}

exports.crawlTVShows = function (limit, onShowFoundEvent) {
    return tumejortorrent.crawlURLsWithTVShows(limit)
        .then(urlList => {
            return _processURLList(urlList, onShowFoundEvent);
        }).catch(err => {
...
exports.crawlMyFavoritesTVShows = function (limit, favoritesList, onShowFoundEvent) {
    // Para cada favorito, obtenemos los shows y componentmoes un show de shows .. o algo asi..
    // TODO ..

    var fnParseShow = function (show) {
        return _doCrawlAndSearchFrom(show);
    }
    var actions = favoritesList.map(fnParseShow);

    return Promise.all(actions)
        .then(shows => { // Cuando se resuelve la promesa, tenemos un array de shows, al que aplicamos una funcion
            mergeTVShowLinks(shows[0], shows);
            //return urlList;
            return shows[0];
        })
    ...
    /*
    return tumejortorrent.crawlEpisodesURL(limit)
        .then(urlList => {
            return _processURLList(urlList, onShowFoundEvent);
        }).catch(err => {
...
function _processURLList(urlList, onShowFoundEvent) {
    ...
    return Promise.all(actions)
        .then(
            showURL => { // Cuando se resuelve la promesa, tenemos un array de shows, al que aplicamos una funcion???
