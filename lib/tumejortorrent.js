...
exports.crawlLinkToURLsWithCollection = function (limit, collectionName) {
    return _crawlLinksFrom(new LinkChained(`${URL_BASE_TVSHOWS_HD}${collectionName}`), limit, 'buscar-list')
}
...
function _crawlLinksFrom(linkChained, limit, classListName) {
    console.log(`_crawlLinksFrom: ${JSON.stringify(linkChained)} - ${limit} - ${classListName}`)
    var linkChainedList = []
