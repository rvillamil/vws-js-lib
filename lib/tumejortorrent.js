...
/**
 * Crawling data from 'http://tumejortorrent.com/series-hd/', with the lastest episode tvshows URLs
 * 
 * @param {*} limit film list limit to return
 * @returns Promise with url list. The URL is a link to the latest episode published
 */
exports.crawlURLsWithTVShows = function (limit) {
    ...
    return crawlShows('http://tumejortorrent.com/series-hd/', limit, 'pelilist')
        ...
        .catch(function (err) {
            console.log('Error on tumejortorrent - crawlURLsWithTVShows: ' + err);
        });
}
...
/**
 * Crawling data for URL in domain 'tumejortorrent.com', with torrent video/films or TVShows files
 * 
...
function _crawlListWithFirstEpisodeURL(urlListWithTVShows) {
    ...
    var fnParseShow = function (urlWithShow) {
        return crawlShows(urlWithShow, 1, 'buscar-list')
    }
