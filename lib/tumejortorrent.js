 ...
 * @param {*} onCrawlFinish If not null, then run on finish process on show list
 */
    ...
    return rp(options)
        .then(function ($) {
            $('.pelilist li a').each(function (index, element) {
                ...
                if (index == 0) {
                    urlList2[index] = {};
                    urlList2[index] = $(element).attr('href');
...
function parseOriginalTitle(str) {
    //const regex = /tulo original\s+([\w\s]+)\w/g;
    const regex = /((tulo original)|(tulo original:))\s+([\w\s]+)\w/g;
    let m, originalTitle;
