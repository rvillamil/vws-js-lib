 ...
 * 
 * @param {*} limit film list limit to return
 * @returns Promise with showLinks object list where every URL, is a link to the latest episode published
 * 
 *  e.g:  
    [{
...
exports.crawlURLsWithLatestTVShows = function (limit) {
    ...
    return _crawlURLsShows(URL_BASE_TVSHOWS_HD, limit, 'pelilist')
        .then(urlsWithLatestTVShows => {
            ...
            return _crawlFirstShowLinksFor(urlsWithLatestTVShows)
        })
...
function _crawlFirstShowLinksFor(urlsWithTVShowsCollection) {
    var showLinksList = [];

    var fnCrawlURLsShows = function (urlWithTVShow) {
        return _crawlURLsShows(urlWithTVShow, 1, 'buscar-list')
    }
    var actions = urlsWithTVShowsCollection.map(fnCrawlURLsShows);
    ...
    return Promise.all(actions).then(urlListWithLatestEpisode => {
        ...
        var index;
        for (index = 0; index < urlListWithLatestEpisode.length; index++) {
            showLinksList[index] = {};
            showLinksList[index].urlCollection = urlsWithTVShowsCollection[index]
            showLinksList[index].urlDataShow = urlListWithLatestEpisode[index]
            //console.log(`showLinksList[index}: ${JSON.stringify(showLinksList[index])}\n`)

        }
        return showLinksList;
    })
}
