...
exports.crawlURLsWithBillboardFilms = function (limit) {
    return _crawlURLsShows(URL_BASE_BILLBOARDFILMS, limit, 'pelilist');
}
...
exports.crawlURLsWithVideoPremieres = function (limit) {
    return _crawlURLsShows(URL_BASE_VIDEOPREMIERES_HD, limit, 'pelilist')
}
 ...
 *  e.g: 
 *  ["http://tumejortorrent.com/descargar/serie-en-hd/barry/temporada-1/capitulo-03/"],
 *  ["http://tumejortorrent.com/descargar/serie-en-hd/shadowhunters/temporada-3/capitulo-04/"],

 */
exports.crawlURLsWithLatestTVShows = function (limit) {

    return _crawlURLsShows(URL_BASE_TVSHOWS_HD, limit, 'pelilist')
        .then(urlsWithLatestTVShows => {
            console.log(`urlsWithLatestTVShows: ${JSON.stringify(urlsWithLatestTVShows)}\n`)
            return _crawlFirstURLFor(urlsWithLatestTVShows)
        })

        .catch(function (err) {
            console.error(`ERROR! - crawlURLsWithLatestTVShows: '${err}'`)
        });
}
...
exports.crawlDataShow = function (urlWithShow) {
    ...
    return rp(options)
        .then(function ($) {
            ...
            // Collection Name
            /*
            var urlSplittedStr = strURLWithShow.split("/")
            ...
            console.log(`SHOW Collection name::  --> ${show.urlBase} -> ${show.collectionName}\n\n`)
*/
            // Title
 ...
 * 
 * @returns Promise with url list with torrent video links
 */
function _crawlURLsShows(url, limit, classListName) {
    ...
    return rp(options)
        ...
        .catch(function (err) {
            console.error(`ERROR! - _crawlURLsShows: '${err}'`)
        });
}

/**
 * Crawl firts torrent link published array, from urls with show data
 * 
 * @param {*} urlsWithTVShows URL array with tvshow data
 * 
 * e.g: urlsWithTVShows:
 ...
 * 
 * @return Promise with array with the latest torrent link published
 *  e.g: 
 *  [http://tumejortorrent.com/descargar/serie-en-hd/barry/temporada-1/capitulo-03/"],
 *  ["http://tumejortorrent.com/descargar/serie-en-hd/shadowhunters/temporada-3/capitulo-04/"],
 *  ["http://tumejortorrent.com/descargar/serie-en-hd/the-good-doctor/temporada-1/capitulo-18/"]
 */
function _crawlFirstURLFor(urlsWithTVShows) {

    var fnCrawlURLsShows = function (urlWithTVShow) {
        return _crawlURLsShows(urlWithTVShow, 1, 'buscar-list')
    }
    var actions = urlsWithTVShows.map(fnCrawlURLsShows);

    return Promise.all(actions).then(urlListWithLatestEpisode => {
        console.log(`urlListWithLatestEpisode: ${JSON.stringify(urlListWithLatestEpisode)}\n`)
