...
exports.crawlDataShow = function(urlWithShow, urlWithCollection) {
    ...
    return rp(options)
        .then(function($) {
            // URL base
            if (strURLWithShow.match('^http://')) {
                strURLWithShow = strURLWithShow.replace("http://", "https://")
            }
            show.urlBase = strURLWithShow
                //console.log(`URL to crawl =${strURLWithShow}`)
            ...
            // Title
            var titleCrawled = $('.page-box h1 strong').text()

            if (titleCrawled) {
                var splittedTitle = $('.page-box h1 strong').text().split("-")
                    //console.log(`splittedTitle =${splittedTitle}`)
                if (splittedTitle.length > 1) {
                    var splittedTextTwo = splittedTitle[0].split("/")
                    if (splittedTextTwo.length > 1) {
                        show.title = splittedTextTwo[1].trim()
                    }
                } else {
                    show.title = splittedTitle[0].trim()
                }
            } else {
                titleCrawled = "not crawled"
            }
