...
exports.crawlLinkToURLsWithEpisodes = function (limit, collectionName) {
    console.log("crawlLinkToURLsWithEpisodes- Collection Naem: " + collectionName)
 ...
 *
 * @param {*} linkChainedToEpisodeList LinkChained with links to Show data and collection to crawl
 * @returns Promise with ShowCollection object with data scraped
 */
exports.crawlDataShowCollection = function (linkChainedToEpisodeList) {
    ...
    return Promise.all(actions)
        .then(shows => {
            ...
            showCollection.url = URL_BASE_TVSHOWS_HD
            shows.forEach(show => {
                showCollection.name = show.collectionName;
 ...
 * @param {*} elementfrom html element whith url list to process
 * 
 * @returns Promise with showLinksList list with torrent video links
 */

function _crawlLinksFrom(linkChained, limit, elementfrom) {
    //console.log(`_crawlLinksFrom: ${JSON.stringify(linkChained)} - ${limit} - ${elementfrom}`)
    var linkChainedList = []
        ...
        .then(function ($) {
            ...
            $(`${elementfrom}`).each(function (index, element) {
                ...
                if (currentHREF.trim() != previousHREF.trim()) {
                    ...
                    linkChainedList[index].current = currentHREF
                    linkChainedList[index].from = linkChained.current
                    */
                    console.log("ANIDADIO: " + JSON.stringify(linkChained))
            ...
            console.log("GENERADA LISTA linkChainedList de elementos: " + linkChainedList.length)
