...
exports.crawlBillboardFilms = function (onShowFound, limit) {
    return crawlShows('http://tumejortorrent.com/estrenos-de-cine/', onShowFound, limit);
}

exports.crawlVideoPremieres = function (onShowFound, limit) {
    return crawlShows('http://tumejortorrent.com/peliculas-x264-mkv/', onShowFound, limit)
}
 ...
 * @returns promise whith 'Shows'
 */
function crawlShows(url, onShowFound, limit) {

    var fnParseShow = function (url) {
        return parseShow(url)
    }

    return parseFilms(url, limit)
        .then(urlItems => {
...
/**
 * Crawling data from 'tumejortorrent' with torrent video files
 * 
