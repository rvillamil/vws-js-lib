...
// Constants
//
const URL_BASE_VIDEOPREMIERES_HD = 'https://pctnew.com/peliculas-x264-mkv/'
const URL_BASE_BILLBOARDFILMS = 'https://pctnew.com/estrenos-de-cine/'
const URL_BASE_TVSHOWS_HD = 'https://pctnew.com/series-hd/'
...
exports.crawlDataShow = function(urlWithShow, urlWithCollection) {
    ...
    return rp(options)
        .then(function($) {
            // URL base
            if (strURLWithShow.match('^https://')) {
                strURLWithShow = strURLWithShow.replace("https://", "http://")
            }
            ...
            // Title
            var htmlChunkWithTitle = $('.page-box h1 strong').text()
            show.title = _parseTitle(htmlChunkWithTitle)
...
function _parseTitle(str) {
    var titleCrawled = "not crawled"
    if (str) {
        var splittedTitle = str.split("-")
            //console.log(`splittedTitle =${splittedTitle}`)
        if (splittedTitle.length > 1) {
            var splittedTextTwo = splittedTitle[0].split("/")
            if (splittedTextTwo.length > 1) {
                titleCrawled = splittedTextTwo[1].trim()
            } else {
                titleCrawled = splittedTextTwo
            }
        } else {
            titleCrawled = splittedTitle[0].trim()
        }
    }
    /*
