...
//
exports.searchShow = function (title, year, kind, debug = false) {
    return _doSearch(title, year, kind, debug)
}
...
function _doSearch(title, year, kind = 'movie', debug = false) {
    var requestOtions = {
    ...
    return rp(requestOtions)
        .then(function (response) {
            ...
            if (response.total_results != 0) {
                var first = _getExactTitleResult(show.title, response, kind)
                ...
                if (first.poster_path) {
                    show.urlwithCover = `http://image.tmdb.org/t/p/w185/${first.poster_path}`
                }
                show.error = 0
            } else {
                show.error = 1
        ...
        .catch((err) => {
            console.error('Response from TMDB: ' + err)
            return err
        })
}
 ...
 */
/*
function _getNearestDateResult(response, kind) {
    var nearestShowByYear = response.results[0]
    var today = new Date()
    var yyyy = today.getFullYear()

    for (var i = 0; i < response.total_results; ++i) {
        var releaseDateShow = null
        if (kind==='tv') {
            releaseDateShow = new Date(response.results[i].first_air_date)
        }
        if (kind==='movie') {
            releaseDateShow = new Date(response.results[i].release_date)
        }
 ...
 * @param {*} response
 */
function _getExactTitleResult(title, response, kind) {
    var nearestShowByYear = response.results[0]
    
    for (var i = 0; i < response.total_results; ++i) {
        //console.log (response.results[i].title)    
        if ( response.results[i].title.toUpperCase() === title.toUpperCase() ){
            nearestShowByYear = response.results[i]
            return nearestShowByYear
        }
    }
