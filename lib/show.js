...
exports.mergeTVShowLinks = function (fromShows, toShow) {
  var i;
  for (i = 1; i < fromShows.length; i++) {
    //firstShow.episodelist.push( shows[i].sesion, shows[i].episode, shows[i].urltodownload)
    tvShowLink = new TVShowLink(
      fromShows[i].sesion,
      fromShows[i].episode,
      fromShows[i].urltodownload)

    toShow.addTVShowLink(tvShowLink)
  }
}