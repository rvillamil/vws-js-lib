 ...
 * @param {string} kind
 */
async function _doSearchShowInXMDB(show, kind, debug = false) {
  // console.log (`_doSearchShowInXMDB for '${show.title}' and year '${show.year}'`)
  // En TMDB por titulo en castellano que es muy fiable
  try {
    const tmdbShow = await tmdb.searchShow(show.title, show.year, kind, debug)
    ...
    const omdbShow = await omdb.searchShow(searchTitle, show.year, kind, debug)
