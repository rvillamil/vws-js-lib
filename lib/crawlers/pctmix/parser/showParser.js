...
exports.parseURLToDownload = function (htmlRawPage) {
  const regex = /(pctmix1.com\/download\/)(.*)";/gm    
  //var str = $.root().html()
  let m, urltodownload
  while ((m = regex.exec(htmlRawPage)) !== null) {
    // This is necessary to avoid infinite loops with zero-width matches
    if (m.index === regex.lastIndex) {
      regex.lastIndex++
    }
    // m[2] = 153751_-1625640900-Black-Lightning--Proper----Temporada-4--HDTV-720p-AC3-5-1.torrent
    // window.location.href = "//pctmix1.com/download/154015_-1625984141-El-lobo-de-Snow-Hollow--2020---BluRay-MicroHD.torrent"
    urltodownload = m[2]        
  }
