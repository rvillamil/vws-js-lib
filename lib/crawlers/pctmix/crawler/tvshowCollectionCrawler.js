...
// NPM modules
//
const tvshowCollectionCrawlerPCTMIX = require('../../../model/showCollection')
...
exports.crawlDataShowCollection = function (urlWithShowCollection, limit) {
  ...
  return showLinksCrawler.crawlTVShowLinksFrom(
    ...
    _doCrawlChainlinks(showCollectionName, linkChainedToEpisodeList)).catch((err) => {
...
function _doCrawlChainlinks(collectionName, linkChainedToEpisodeList) {
  const actions = linkChainedToEpisodeList.map((linkChained) => tvshowCrawler.crawlDataTVShow(linkChained.from))

  // eslint-disable-next-line no-undef
  return Promise.all(actions).then((shows) => {
    const showCollection = new tvshowCollectionCrawlerPCTMIX()
