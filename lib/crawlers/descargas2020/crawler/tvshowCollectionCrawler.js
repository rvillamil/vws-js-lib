...
exports.crawlDataShowCollection = function (urlWithShowCollection, limit) {
    ...
    const strURLWithShowCollection = urlWithShowCollection + ''
    console.log(`${Constants.DOMAIN} - crawlDataShowCollection on url '${strURLWithShowCollection}'`)
    showCollection.name = tvshowCollectionParser.parseUrlWithShowCollectionName(strURLWithShowCollection)
    ...
    return showLinksCrawler.crawlLinksFrom(
        ...
        console.log(`${Constants.DOMAIN} - crawlDataShowCollection - ${linkChainedList.length} episodes found`)
        //console.log(`linkChainedList --> ${JSON.stringify(linkChainedList)}`)
        return _doCrawlChainlinks(showCollection.name, linkChainedList)
    }).catch(err => {
