 ...
 * <script type="application/ld+json">
{
  ...
  then, get the string: "Watchmen - Temporada 1 [HDTV 720p][Cap.106][AC3 5.1 Castellano][www.descargas2020.ORG][www.pctnew.ORG]"
}
 */
exports.parseSession = function (htmlFragment) {

    //console.log(`TODO: ${htmlFragment}`)
...
exports.parseEpisode = function (htmlFragment, session) {
    let episode
    var matches = htmlFragment.match('Cap.' + session + '(\\d+)')
    if (matches) {
        episode = matches[1]
    } else {
        matches = htmlFragment.match('Cap.' + '(\\d+)')
        episode = matches[1]
    }
    return episode
}
