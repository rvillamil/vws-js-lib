...
class FavoriteRepository {
	...
	updateCollectionWithNewShow(newShowCollectionName, newShow) {
		...
		return this.findShowByURLtodownload(newShow.urltodownload).then(
			docFound => {
				if (!docFound) {
					console.log(`updateCollectionWithNewShow - Show ${newShow}'not found!!`)
					return newShow
				} else {
	...
	updateCollectionWithNewShows(newShowCollectionName, newShows) {
		var reverseShows = newShows.reverse()
		/*
		reverseShows.forEach(show => {
			console.log(`The show to replace2: ${show.currentSession} - ${show.currentEpisode}`)
		})
		*/
		var actions = reverseShows.map(showToAdd => {
